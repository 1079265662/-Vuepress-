...
* <font color = #ff3040>Vue3中的生命周期(钩子函数)</font>
  ...
  * `onUnmounted(()=>{ })`  卸载销毁后
* 所有的声明周期方法 都可以在`setup()` 里面多次调用 (一样声明周期按js默认顺序排序)
...
* 单独定义响应式数据(在多个响应式中 提取单独一个的值)
  - `toRef()` 和 `reactive()` 相互关联 
    - `reactive()` 定义了多个响应式 如果想单独调用 需要用到`toRef()` 单独调用出来(需要`return`返回)
...
<script>
// 按需导入reactive()方法 和 toRef() 方法
import { reactive, toRef } from 'vue'
export default {
  setup (props) {
    // 设置reactive()的ret响应式数据
    const ret = reactive({
      name: '你好',
      age: '500'
    })
    // toRef()只导出响应式数据ret 里面的name值
    const name = toRef(ret, 'name')
        // 设置切换的方法
    const change = () => {
      if (ret.name === '万岁') {
        ret.name = '你好'
...
## 组合API `toRefs()` 全部导出响应数据(去除前缀)

> 目标：掌握使用toRefs函数定义转换响应式中**所有**属性为响应式数据，通常用于解构|展开reactive定义对象。
>
> 使用场景：把`reactive()`里面的所有响应式数据 进行总拆分时候使用

* <font color = #ff3040>Vue中导入方法 需要用 import按需加载 才可以在 `setup()`里面使用该方法</font>
* 多个定义响应式数据(在多个响应式中 提取所有的值)
  - `toRefs()` 和 `reactive()` 相互关联 
    - `reactive()` 定义了多个响应式 如果调用所欲 需要用到`toRefs()` 调用所有的出来 (需要`return`返回 并且 需要 `...`展开语法)
...
export default {
  setup (props) {
    ...
    // toRefs()方法把多个reactive()的值提取出来
    const rets = toRefs(ret)
    // 设置切换的方法
    const change = () => {
      if (ret.name === '万岁') {
        ret.name = '你好'
    ...
    // 把toRefs()方法提取的值 展开语法的方式导出 return返回
    return { change, ...rets }
  }
...
* 子组件接收 `props` 父组件传来的值 需要设置 `toRef`或`toRefs` 把接收到的数据 变为响应式(不可以用`ref`)
* <font color =#4ac18b> `v-model:` 有默认值 `modelValue` 默认值绑定的话 不需要设置名称 但是子组件接收需要写  `modelValue`  </font>
