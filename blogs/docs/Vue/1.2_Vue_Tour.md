...
* 如果你ui是一个好说的不严格的人 你可以使用他的默认按钮显示 比如说保留结束引导这个按钮 就可以不用通过插槽自定义按钮内容了 只需要统一改名称即可 你可以到[这里看看](https://github.com/pulsardev/vue-tour/wiki/Tour-Configuration) 我介绍的是自定义插槽的效果 默认他的按钮是自带结束引导的(我们项目不让带) 所以用到了这个组件的插槽功能
